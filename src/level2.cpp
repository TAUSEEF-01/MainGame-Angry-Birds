...
void level2(SDL_Renderer *renderer, bool &quit, State &currentState, int &musicPlaying, std::string &playerName, int &playerScore, int &turns)
{
    ...
    bool jump = 0, Green = 1, green_go_up = 1, clicked = 0; // clikcked
    int Bird_length = 50;
    ...
    while (!quit)
    {
        ...
        if (jump && speed_x)
        {
            ...
            if (bird_rect.x - 50 >= (SCREEN_WIDTH) || (bird_rect.x + Bird_length + 100 <= 0))
            {
                ...
                if (Green == 0) // Next Level + // If bird got hit
                {
                    playerScore += 10;
            ...
            if ((bird_rect.y >= (SCREEN_HEIGHT - 5 * Bird_length + 70))) // (bird_rect.y <= 0)
            {
                ...
                speed_y *= 0.60; // speed of y axis reduces
                speed_x *= 0.90; // speed of x axis reduces
    ...
    here:
        ...
        if (!collide2(bird_rect, green_bird_rect) && Green) // enemy bird rendering procedure
        {
            SDL_RenderCopy(renderer, green_bird, NULL, &green_bird_rect);

            // SDL_Delay(10);

            if (green_go_up) // green bird up down motion - start
            {
                // green_bird_rect.y -= (jump?6:1);
                green_bird_rect.y -= 6;

                if (green_bird_rect.y <= 90)
                    green_go_up = 0;
            }
            else
            {
                // green_bird_rect.y += (jump?6:1);
                green_bird_rect.y += 6;
                if (green_bird_rect.y + green_bird_rect.h >= SCREEN_HEIGHT - 95)
                    green_go_up = 1;
            } // green bird up down motion - end
        }
        else
        {
            Green = 0;
