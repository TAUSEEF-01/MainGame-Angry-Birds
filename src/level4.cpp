...
SDL_Texture *currentButtonTexture4; // Variable to hold the current button texture

// Function to toggle music on/off
void toggleMusic4(SDL_Texture *muteTexture, SDL_Texture *unmuteTexture, int &musicPlaying)
{
    if (musicPlaying)
    {
        Mix_PauseMusic();                      // Pause the music
        musicPlaying = 0;                      // Update the flag to indicate music is stopped
        currentButtonTexture4 = unmuteTexture; // Set the texture to unmute button
    }
    else
    {
        Mix_ResumeMusic();                   // Resume the music
        musicPlaying = 1;                    // Update the flag to indicate music is playing
        currentButtonTexture4 = muteTexture; // Set the texture to mute button
    }
}

bool collide4(SDL_Rect a, SDL_Rect b)
{
...
void level4(SDL_Renderer *renderer, bool &quit, State &currentState, int &musicPlaying, std::string &playerName, int &playerScore, int &turns)
{
    ...
    while (!quit)
    {
        while (SDL_PollEvent(&e))
        {
            ...
            if (e.type == SDL_MOUSEMOTION)
            {
                ...
                {
                    SDL_SetTextureColorMod(currentButtonTexture4, 255, 255, 255);
                }
                else
                {
                    SDL_SetTextureColorMod(currentButtonTexture4, 150, 150, 150);
                }
            }
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN)
            {
                    ...
                    Y >= playButtonRect.y && Y <= (playButtonRect.y + playButtonRect.h))
                {
                    ...
                    currentButtonTexture4 = unmuteButtonTexture;
                    musicPlaying = 1;
                ...
                if (X >= musicButtonRect.x && X <= (musicButtonRect.x + musicButtonRect.w) &&
                    Y >= musicButtonRect.y && Y <= (musicButtonRect.y + musicButtonRect.h))
                {
                    toggleMusic4(muteButtonTexture, unmuteButtonTexture, musicPlaying);
                    // SDL_Delay(150);
                }
            }
        }
        ...
        if (musicPlaying)
        {
            currentButtonTexture4 = muteButtonTexture;
        }
        else
        {
            currentButtonTexture4 = unmuteButtonTexture;
    ...
    here:
        ...
        if (!collide4(bird_rect, green_bird_rect) && Green)
    ...
    SDL_RenderCopy(renderer, currentButtonTexture4, NULL, &musicButtonRect);
    SDL_RenderPresent(renderer);
