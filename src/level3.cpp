...
SDL_Texture *currentButtonTexture3; // Variable to hold the current button texture
...
void toggleMusic3(SDL_Texture *muteTexture, SDL_Texture *unmuteTexture, int &musicPlaying)
{
    if (musicPlaying)
    {
        ...
        currentButtonTexture3 = unmuteTexture; // Set the texture to unmute button
    }
    else
    {
        Mix_ResumeMusic();                   // Resume the music
        musicPlaying = 1;                    // Update the flag to indicate music is playing
        currentButtonTexture3 = muteTexture; // Set the texture to mute button
    }
}

bool collide3(SDL_Rect a, SDL_Rect b)
{
...
void level3(SDL_Renderer *renderer, bool &quit, State &currentState, int &musicPlaying, std::string &playerName, int &playerScore)
{
    ...
    while (!quit)
    {
        while (SDL_PollEvent(&e))
        {
            ...
            if (e.type == SDL_MOUSEMOTION)
            {
                ...
                {
                    SDL_SetTextureColorMod(currentButtonTexture3, 255, 255, 255);
                }
                else
                {
                    SDL_SetTextureColorMod(currentButtonTexture3, 150, 150, 150);
                }
            }
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN)
            {
                    ...
                    Y >= playButtonRect.y && Y <= (playButtonRect.y + playButtonRect.h))
                {
                    ...
                    currentButtonTexture3 = unmuteButtonTexture;
                    musicPlaying = 1;
                ...
                {
                    toggleMusic3(muteButtonTexture, unmuteButtonTexture, musicPlaying);
                    // SDL_Delay(150);
                }
            }
        }
        ...
        if (musicPlaying)
        {
            currentButtonTexture3 = muteButtonTexture;
        }
        else
        {
            currentButtonTexture3 = unmuteButtonTexture;
        ...
        if (jump && speed_x)
        {
            ...
            if (bird_rect.x - 50 >= (SCREEN_WIDTH) || (bird_rect.x + Bird_length + 100 <= 0))
            {
                ...
                {
                    playerScore += 10;
                    currentState = LEVEL4_LOADING_PAGE; // added extra
    ...
    here:
        ...
        if (!collide3(bird_rect, green_bird_rect) && Green)
    ...
    SDL_RenderCopy(renderer, currentButtonTexture3, NULL, &musicButtonRect);
    SDL_RenderPresent(renderer);
